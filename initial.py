...
def init():
    ...
    for i in train_sen:
        ...
        for j in range(lenth):
            #train_sen为字典，train_sen[i]为一个实体对对应存储的内容，即多个标签及多个句子，标签在前
            #所以train_x增加的是每一个标签对应的多个句子
...
def seperate():
    ...
    #所有训练数据，i即为每一个训练元素（一个标签对应的多个句子）
    for i in range(len(x_train)):
        word = []
        pos1 = []
        pos2 = []
        #j为第i对实体对对应的多个句子，取每一个句子
        for j in x_train[i]:
            temp_word = []
            temp_pos1 = []
            temp_pos2 = []
